...
{
    public async Task<Either<Error, Product>> Add(Product product, CancellationToken cancellationToken)
    {
    ...
    public async Task<Either<Error, Product>> Update(Product product, CancellationToken cancellationToken)
    {
    ...
    public async Task<Option<Error>> Delete(Guid productId, Guid userId, CancellationToken cancellationToken)
    {
        var existing = await connection.GetSingleWhere<Product>(nameof(Product), where: new Dictionary<string, object>
        {
    ...
    public async Task<Option<Error>> MarkAsInCart(Guid productId, Guid userId, CancellationToken cancellationToken)
    {
        var existing = await connection.GetSingleWhere<Product>(nameof(Product), new Dictionary<string, object>
        {
            [nameof(Product.Id)] = productId,
            [nameof(Product.UserId)] = userId
        });
        ...
        return updated ? Option<Error>.None : new ProductUpdateFailedError(product.UserId, product.Id);
    }

    public async Task<Option<Error>> RemoveFromCart(Guid productId, Guid userId, CancellationToken cancellationToken)
    {
        var existing = await connection.GetSingleWhere<Product>(nameof(Product), new Dictionary<string, object>
        {
            [nameof(Product.Id)] = productId,
